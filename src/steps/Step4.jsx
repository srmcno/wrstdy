...
export function Step4({ study, onGoToStep }) {
  ...
  const scorecard = [
    { metric: 'Budget Coverage Ratio', cur: fmt.ratio(curOR), prop: fmt.ratio(propOR), benchmark: '≥ 1.25',
    ...
    { metric: 'Affordability Index', cur: fmt.pd(curAI, 'N/A'), prop: fmt.pd(propAI, 'N/A'), benchmark: '< 2.00%',
      ...
      curStatus: curAI == null ? 'Enter MHI' : (curAI < 0.02 ? 'Below screen' : 'Burden'),
      propStatus: propAI == null ? 'Enter MHI' : (propAI < 0.02 ? 'Below screen' : 'Burden'),
      guide: 'Cost of 5,000 gal ÷ Monthly MHI. Planning screen only; assess low-income household impacts separately.' },
    { metric: 'Debt Service Coverage (DSCR)', cur: fmt.ratio(curDSCR, 'No debt'), prop: fmt.ratio(propDSCR, 'No debt'), benchmark: '≥ 1.25',
      ...
      curStatus: curDSCR == null ? 'No debt' : curDSCR >= 1.25 ? 'Above planning target' : curDSCR >= 1.1 ? 'Thin margin' : 'Below planning target',
      propStatus: propDSCR == null ? 'No debt' : propDSCR >= 1.25 ? 'Above planning target' : propDSCR >= 1.1 ? 'Thin margin' : 'Below planning target',
